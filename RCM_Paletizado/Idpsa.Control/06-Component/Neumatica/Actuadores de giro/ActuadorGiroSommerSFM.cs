...
{
    [System.Serializable()]
    public class ActuadorGiroSommerSFM:IChainControllersOwner,IManualsProvider  
    {
        private ChainController _chainController;
        private ActuadorConInversorSimple _electroValvula1;
        private ActuadorConInversorSimple _electroValvula2;
        private TON _timer;
        private const int _periode=350;
        ...
        private void Activate(ActuadorConInversorSimple electroValvula, v value, bool invert)
        {
            if (invert)
                value = Invert(value);
        ...
        private static PairList<v,v> s270_90=
            new PairList<v, v>() { { v.b, v.o }, { v.b, v.o }, { v.b, v.a }, { v.a, v.a }, { v.a, v.a }, { v.a, v.o } };

        private static PairList<v, v> s270_0 = new PairList<v,v>() {{v.b,v.o},{v.a,v.o}};

        private static PairList<v,v> s0_270=
            new PairList<v, v>() { { v.b, v.o }, { v.b, v.o }, { v.o, v.b }, { v.o, v.b }, { v.b, v.b }, { v.b, v.b }, { v.b, v.o } };
        ...
        public ActuadorGiroSommerSFM(IBitEvaluable sensor_270º, IBitEvaluable sensor_0º, IBitEvaluable sensor_90º,

            ActuadorConInversorSimple electroValvula1, ActuadorConInversorSimple electroValvula2, string parName)
        {
            Name = parName;
        ...
        public void GiroChain(Spin spin)
        {
            _chainController.CallChain("giro")
                .WithParam("spin", spin);
        }
      
        private void GiroChain_Steps()
        {
            ...
            chain.Add(new Step(0,5000, "Comprobar posición actual giro pinza válida")).Task = () =>
            {
                currentSpin = EnGiro();
            ...
            chain.Add(new Step(10, () => "Realización paso " + (index + 1) + " giro pinza " + currentSpin.ToStringImage() +" -> " + targetSpin.ToStringImage())).Task = () =>
            {
        ...
        private Spin EnGiro(){
            if (EnGiro(Spin.S0)) return Spin.S0;
            else if (EnGiro(Spin.S90)) return Spin.S90;
            else if (EnGiro(Spin.S270)) return Spin.S270;
        ...
        {         
            DynamicManual dynamicManual = new DynamicManual(this)   
                         ...
                         .WithLeftStep(new DynamicStepBody(() => "Girar -90", ()=>GiroChain(Spin.S270)))
                         .WithLeftStepName("-90º")
                         .WithLeftCondition(_sensor_270)

                         .WithCenterStep(new DynamicStepBody(() => "Girar 0º",()=> GiroChain(Spin.S0)))
                         .WithCenterStepName("0º")
                         .WithCenterCondition(_sensor_0)

                         .WhitRightStep(new DynamicStepBody(() => "Girar +90", ()=> GiroChain(Spin.S90)))
                         .WhitRightStepName("+90º")
                         .WithRightCondition(_sensor_90);              

           return new[] { new Manual(dynamicManual){Descripcion=Name} };
        }
