...
namespace Idpsa.Control.User
{
    ...
    {       
        private static readonly UserAccess _instance = new UserAccess();
        ...
        public bool CurrentUserEqualOrSuperior(UserType value)
        {
        ...
        public UserType GetUserType(string user, string pass)
        {
            UserType TipoDeUsuario = UserType.None;
            if (UserKeys.ContainsKey(user))
            {
        ...
        public bool SetUserType(string user, UserType TipoDeUsuario)
        {
            if (UserKeys.ContainsKey(user))
            {
        ...
        private string DesencriptaDato(string strDato)
        {
            ...
            try
            {
                while ((lTotal >= lRead))
                {
                    ...
                    long lReadNow = (int)(((double)bPlain.Length / (double)descsp.BlockSize) * descsp.BlockSize);
        ...
        public struct PasswordPrivilegio
        {
            ...
            public UserType Privilegio;

            public PasswordPrivilegio(string password, string privilegio)
            {
                Password = password;
                Privilegio = (UserType)Enum.Parse(typeof (UserType), privilegio, true);
            }
        }
