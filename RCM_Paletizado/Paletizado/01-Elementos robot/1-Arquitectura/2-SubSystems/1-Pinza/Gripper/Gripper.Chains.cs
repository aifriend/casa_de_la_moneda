...
namespace Idpsa.Paletizado
{
    public partial class Gripper
    {
        ...
        public bool CheckPalletStorPosition(Point3D pos) //MCR.
        {
            if (pos.X != _sys.Geo.PosPaletStore1.X && pos.X != _sys.Geo.PosPaletStore2.X)
            {
                return false;
            }
            else if (pos.Y != _sys.Geo.PosPaletStore1.Y && pos.Y != _sys.Geo.PosPaletStore2.Y)
                return false;
            else
            {
                return true;
        ...
        private void SequentialSetCheck_Steps()
        {
            ...
            chain.Add(new Step(() => "Alejar Verja", 5000)).Task =
                    () =>
                    {
                        if (!EnGiro(chain.Param<Spin>("spin")))
                        {
                            if (_gantry.Y.Posicion > 1750)
                                _gantry.Y.StartMov(1750, 800,800);
                        }
                        _chainController.NextStep();
                    };
            ...
            chain.Add(new Step(() => "Comprobar pinza en " + chain.TryParam<Spin>("spin").ToStringImage(), 12000)).Task =
                () =>
                    {
                        if (EnGiro(chain.Param<Spin>("spin")))
                        {
                        ...
                        {
                            GirarChain(chain.Param<Spin>("spin"));
                        }
                    };
